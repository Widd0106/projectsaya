...
import { Sparkles, User, Lock, Eye, EyeOff } from 'lucide-vue-next';
    ...
    <div class="w-full max-w-3xl">
      <div class="mb-8 flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-400">Welcome back to your story.</p>
        </div>
        <div class="flex gap-3">
          <button class="rounded-full bg-gradient-to-r from-accent to-accent-light px-6 py-2 text-sm font-semibold text-bg shadow-lg shadow-accent/20">
            LOGIN
          </button>
          <Link
            :href="route('register')"
      ...
      <p class="mt-6 text-center text-sm text-gray-400">
        ...
        <Link :href="route('register')" class="font-medium text-accent hover:underline">Create Account</Link>
      </p>
    </div>
  </div>
</template>
